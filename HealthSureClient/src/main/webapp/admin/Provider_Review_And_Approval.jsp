<!-- 
  ...
  Purpose: This page is designed to display a table of providers for admin review. 
  Administrators can view and validate the provider details. 
  The page also provides pagination functionality and options to go back to the admin dashboard.

  Components:
  - Table: Displays provider details like ID, name, email, hospital, status, etc.
  - Validation Button: A button for validating providers, which can be enabled/disabled based on the provider's status.
  - Pagination Controls: Links for navigating through pages of providers.
  - Footer: Displays copyright information at the bottom of the page.

  Design:
  - The page is designed with a modern, clean interface using pastel colors (e.g., #E8F5E9 background, #00796B buttons).
  - A flexible table layout is used to display provider details, with clickable column headers for sorting.
  - A "Validate" button allows admins to validate providers and change their status to "Validated".
  - The use of `h:dataTable` ensures data is displayed dynamically, linked to managed beans in the backend.
-->
...
.disabled-button:hover {
    background-color: #B2DFDB !important;  /* Ensure background stays the same */
    color: black;           
    transform: none !important;            /* Disable any hover effects (like scaling) */
}
/* Styling for Validated Buttons */
.validated-button {
    background-color: #B2DFDB !important;  /* Lighter background similar to disabled */
    color: black;          
