...
//	}
	...
	public String showDoctorDetails() {
		...
		try {
			...
			this.doctor = doctorDao.getDoctorById(doctor.getDoctorId());
			if (this.doctor != null) {
	...
	public String showProviderDetails() {
		...
		try {
			...
			this.provider = doctorDao.getProviderById(provider.getProviderId());
...
//		doctorsList.forEach(System.out::println);
	...
	public void validateProvider(Provider provider) {
		...
		if (isValid) {
			...
			MailSend.sendInfo(provider.getEmail(), "PROVIDER REQUEST APPROVED",
			...
			MailSend.sendInfo(provider.getEmail(), "PROVIDER REQUEST REJECTED",
	...
	public String search() {
		...
		if ("doctor".equals(searchType)) {
			...
			searchProviders(); // You can implement similar logic for Providers
			System.out.println("Provider Search Results: " + searchResultsP);

		} else {
			searchResults = Collections.emptyList();
			searchResultsP = Collections.emptyList(); // Empty if input or criteria are missing
		}
		// If no validation error and results found, redirect to results page
	...
	public void searchDoctors() {
		if (searchCriteria != null && !searchCriteria.isEmpty() && searchInput != null && !searchInput.isEmpty()) {
			...
			case "doctorName":
				...
				} else if (searchInput.matches(".*\\d.*")) { // Check if the input contains any digits
					isValid = false;
					errorMessage = ErrorMessage.INVALID_DOCTOR_NAME;
				}
				break;
	...
	private void searchProviders() {
		if (searchCriteria != null && !searchCriteria.isEmpty() && searchInput != null && !searchInput.isEmpty()) {
			...
			case "providerName":
				...
				} else if (normalizedInput.matches(".*\\d.*")) { // Check if the input contains any digits
					isValid = false;
					errorMessage = ErrorMessage.PROVIDER_NAME_NUMBER;
				}
				break;
